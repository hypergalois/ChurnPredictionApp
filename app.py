...
# If anyone has trouble with the pickle version:
# https://github.com/scikit-learn/scikit-learn/issues/28666

st.title("Predecir Abandono de Clientes 🚀")
st.write(
    "¡Haz clic en Predecir para determinar si es probable que tu cliente abandone!"
)
...
for column, column_properties in schema["column_info"].items():
    if column == "churn":
        ...
        options[column] = st.sidebar.slider(
            column, min_val, max_val, value=feature_mean
        )
...
if st.button("Predecir"):
    ...
    scoring_sample = transform_data(
        scoring_data, column_order_out, mean_eve_mins, onehot
    )
...
try:
    ...
    if os.path.isfile("./data/historical_data.csv"):
        historical.to_csv(
            "./data/historical_data.csv", mode="a", header=False, index=False
        )
    else:
        historical.to_csv("./data/historical_data.csv", header=True, index=False)
